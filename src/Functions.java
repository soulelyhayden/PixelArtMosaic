...
class Functions {
    double map(double x, double cMin, double cMax, double nMin, double nMax) {
    ...
    double randomGaussian(double standDev, double mean) {
        return ((ThreadLocalRandom.current().nextGaussian() * standDev) + mean);
    }

    Color randomCol(Color cCol) {
        int r, g, b;
        r = (int)(constrain((random() + 1) * cCol.getRed(), 0 , 255));
        g = (int)(constrain((random() + 1) * cCol.getGreen(), 0 , 255));
        b = (int)(constrain((random() + 1) * cCol.getBlue(), 0 , 255));
    ...
    double percentDifference(double val1, double val2) {
        if (val1 > 0) {
    ...
    double lerp(double amount, double left, double right) {
    ...
    double PerlinNoise(double x, double y, double smoothness, int rep) {
        float total = 0;
